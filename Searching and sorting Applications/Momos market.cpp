...
int main()
{
    ...
    //copying into prefix array
    //int prefix[n]={0};
    for(int i=1;i<=n;i++)
    {
       querry[i]+=querry[i-1];
    }
    //applying binary search in the prefix array
    int q;
    cin>>q;
    while(q--){
        int total_money;
        ...
        if(total_money<querry[0]){
            cout<<0<<" "<<total_money<<endl;
            continue;
        }
    int ans=0;
    ...
    cout<<ans+1<<" "<<total_money-querry[ans]<<endl;
...
After buying vegetables, she goes to "Momos Market", where there are ‘n’ number of shops of momos. Each of the shops of momos 
has a rate per momo. She visits the market and starts buying momos (one from each shop) starting from the first shop. She will
visit the market for ‘q’ days. You have to tell that how many momos she can buy each day if she starts buying from the first 
shop daily. She cannot use the remaining money of one day on some other day. But she will save them for other expenses in the
future, so, you also need to tell the sum of money left with her at the end of each day.
Input Format:
